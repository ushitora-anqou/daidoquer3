...
export class MusicSubscription {
  ...
  public constructor(voiceConnection: VoiceConnection) {
    ...
    // this.voiceConnection.on('stateChange', (oldState, newState) => {console.log(`voiceConnection transitioned from ${oldState.status} to ${newState.status}`);})
  ...
  private async processQueue(): Promise<void> {
    ...
    try {
      ...
      // retry for n-times
      // FIXME: キューに追加してリトライしてごまかすのをやめろ
      if (nextTrack.retryTime > 0) {
        nextTrack.onRetry(nextTrack.retryTime);
        await wait(10_000);
        this.queue.unshift(nextTrack);
        nextTrack.decRetryTime();
      } else {
